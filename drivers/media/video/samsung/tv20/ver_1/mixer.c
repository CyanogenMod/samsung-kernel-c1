...
int s5p_vmx_set_layer_priority(enum s5p_tv_vmx_layer layer, u32 priority)
{
	...
	case VM_VIDEO_LAYER:
		layer_cfg = S5P_MXR_LAYER_CFG_VP_PRIORITY_CLR(
				...
				S5P_MXR_LAYER_CFG_VP_PRIORITY(priority);
		break;
...
void s5p_vmx_set_ctrl(enum s5p_tv_vmx_layer layer,
			bool premul,
			bool pixel_blending,
			bool blank_change,
			bool win_blending,
				...
				u32 dst_height)
{
	...
	case VM_GPR0_LAYER:
		...
		s5p_vmx_set_grp_layer_size(layer, span, width, height,
					    src_offs_x, src_offs_y);

		s5p_vmx_set_grp_base_address(layer, base_addr);
		s5p_vmx_set_grp_layer_position(layer, dst_offs_x,
			dst_offs_y);

		temp_reg = readl(mixer_base + S5P_MXR_GRAPHIC0_WH);
		h_factor = s5p_vmx_grp_scaling_factor(width, dst_width, 1);
		v_factor = s5p_vmx_grp_scaling_factor(height, dst_height, 0);
	...
	case VM_GPR1_LAYER:
		...
		s5p_vmx_set_grp_base_address(layer, base_addr);
		s5p_vmx_set_grp_layer_position(layer, dst_offs_x, dst_offs_y);
	...
	s5p_vmx_set_layer_show(layer, show);
