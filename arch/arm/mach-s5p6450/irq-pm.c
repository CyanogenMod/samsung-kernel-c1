/* arch/arm/mach-s5p6450/irq-pm.c
 *
 ...
 *
 * S5P6450 - Interrupt handling Power Management
 *
...
static struct sleep_save irq_save[] = {
	SAVE_ITEM(S5P6450_PRIORITY),
	SAVE_ITEM(S5P6450_EINT0CON),
...
static int s5p6450_irq_pm_suspend(struct sys_device *dev, pm_message_t state)
{
	...
	for (i = 0; i < ARRAY_SIZE(eint_grp_save); i++, grp++) {
		...
		grp->mask = __raw_readl(S5P6450_EINT12MASK + (i * 4));
		grp->fltcon = __raw_readl(S5P6450_EINT12FLTCON + (i * 4));
	}

	return 0;
}

static int s5p6450_irq_pm_resume(struct sys_device *dev)
{
	struct irq_grp_save *grp = eint_grp_save;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(eint_grp_save); i++, grp++) {
		...
		__raw_writel(grp->mask, S5P6450_EINT12MASK + (i * 4));
		__raw_writel(grp->fltcon, S5P6450_EINT12FLTCON + (i * 4));
	}
...
{
	return sysdev_driver_register(&s5p6450_sysclass, &s5p6450_irq_driver);
}

arch_initcall(s5p6450_irq_pm_init);
